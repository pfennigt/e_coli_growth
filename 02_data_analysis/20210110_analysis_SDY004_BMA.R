...
names(pre_means) = as.character(0:6)
...
plts$drymass_weights <- 
  ggplot(analysis_weights[grepl("post", analysis_weights$variable),]) +
    theme_bw() +
...
# Estimate drymass from scatter
# Force 0 intercept to avoid negative drymasses
drymass_lms <- list(
  description = "estimate dry mass [mg ml^-1] from scatter [AU]; values before the scatter drop with lm1, no values after the scatter drop",
  estimator = "scatter",
  lm1 = lm(
    norm_diff_mgml ~ 0+scatter,
    summary_weights[summary_weights$growth_phase == "1. growth",]
  ),
  lm2 = NA
)

drymass_lms$lm1$coefficients <- c("(Intercept)" = 0, drymass_lms$lm1$coefficients)
...
drymass_lms_coefs$value_type <- "scatter"
...
scatter_means = rowMeans(getData(dats$biolector$data, "scatter")[, reference_wells])

drymass_lms_predicts = data.frame(
  pred = estimate_two_phases(
    values = scatter_means,
    times = times,
...
plts$drymass_vs_scatter_ribof <- 
  plts$drymass_vs_scatter_ribof +
    ...
    geom_label(
      data = data.frame(value_type = "scatter"),
...
drymass_lms_filename = paste(
  dat$study_id,
  dat$study_token,
  "drymass_estimation_twophase_scatter.rds",
  sep = "_"
)
...
# Integrate control measurements  ---------------------------
...
plts$control_filter_weights_raw <- 
  ggplot(sample_weights, aes(x=NA, y=value, color=variable)) + 
  theme_bw() +
  geom_boxplot() +
  geom_point(position=position_dodge(0.75)) +
...
# Plot the calculated control dry mass
plts$control_drymass_weights <- 
  ggplot(analysis_weights[grepl("post", analysis_weights$weight_type),]) +
  theme_bw() +
  geom_hline(yintercept = 0, linetype  = "dashed") +
  
  geom_boxplot(aes(x = sample_id, y = drymass_cmmoll)) +
  geom_point(aes(x = sample_id, y = drymass_cmmoll)) +
  labs(x = "control sample", y = "control dry mass [C-mmol"~l^-1*"]", color = "Dryweight\nmeasurement")
